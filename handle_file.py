...
def handle_file(file_buffer, filetype_buffer, filename_buffer, bot_name, pinecone_index, tokenizer):
    """Handle a file by extracting its text, creating embeddings, and upserting them to Pinecone."""
    ...
    try:
        extracted_text = extract_text_from_file(file_buffer, filetype_buffer)
    ...
    return handle_file_string(filename, bot_name, extracted_text, pinecone_index, tokenizer, file_text_dict)

# Extract text from a file based on its mimetype
def extract_text_from_file(file_buffer, filetype_buffer):
    #check the file_buffer type
    # mime_type, _ = mimetypes.guess_type(None, strict=False, buffer=file_buffer)

    """Return the text content of a file."""
    if filetype_buffer == "application/pdf":
        # Extract text from pdf using PyPDF2
        reader = PdfReader(file_buffer)
        extracted_text = ""
        for page in reader.pages:
            extracted_text += page.extract_text()
    elif filetype_buffer == "text/plain":
        # Read text from plain text file
        extracted_text = file_buffer.read().decode("utf-8")
        file_buffer.close()
    elif filetype_buffer == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
        # Extract text from docx using docx2txt
        extracted_text = docx2txt.process(file_buffer)
    else:
        # Unsupported file type
        raise ValueError("Unsupported file type: {}".format(filetype_buffer))

    return extracted_text

# Handle a file string by creating embeddings and upserting them to Pinecone
def handle_file_string(filename, bot_name, file_body_string, pinecone_index, tokenizer, file_text_dict):
    """Handle a file string by creating embeddings and upserting them to Pinecone."""
    ...
    for i, (text_chunk, embedding) in enumerate(text_embeddings):
        id = get_pinecone_id_for_file_chunk(bot_name, filename, i)
        file_text_dict[id] = text_chunk
        vectors.append(
            (id, embedding, {"filename": filename, "file_chunk_index": i,"user_id": user_id, "text_chunk": text_chunk }))
        logging.info(
            "[handle_file_string] Text chunk {}: {}".format(i, text_chunk, user_id))
    ...
    for batch in batches:
        try:
            pinecone_index.upsert(
                vectors=batch, namespace=bot_name)
...
def create_embeddings_for_text(text, tokenizer):
    ...
    total_cost = ((total_count)/1000) * 0.0004
